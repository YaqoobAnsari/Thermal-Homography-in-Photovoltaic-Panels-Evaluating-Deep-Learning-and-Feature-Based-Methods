...
BRISK (Binary Robust Invariant Scalable Keypoints) based homography estimation.

BRISK Features:
- Binary descriptor (very fast matching)
- Scale and rotation invariant
- Faster than SIFT/SURF
- Good for real-time applications
- Patent-free
- Lower accuracy than SIFT but much faster
...
RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results', 'Traditional_methods_results', 'BRISK')


class BRISKHomographyEstimator(TraditionalHomographyEstimator):
    """
    BRISK-based homography estimator.
    """

    def __init__(self, thresh=30, octaves=3, patternScale=1.0,
                 use_flann=True, ransac_reproj_threshold=5.0, ransac_method='RANSAC',
                 use_advanced_matching=True, distance_ratio=0.75):
        """
        Initialize BRISK detector.

        Args:
            thresh: AGAST detection threshold (lower = more features)
            octaves: Detection octaves (0 = single scale)
            patternScale: Scaling factor for sampling pattern
        ...
        super().__init__('BRISK', use_flann, ransac_reproj_threshold,
...
def main():
    """
    Run BRISK homography estimation on all data types.
    """
    print("="*80)
    print("BRISK-BASED HOMOGRAPHY ESTIMATION")
    print("="*80)

    # Create BRISK estimator
    estimator = BRISKHomographyEstimator(
        thresh=30,                # AGAST threshold
        octaves=3,                # Scale space octaves
        patternScale=1.0,         # Standard pattern scale
        use_flann=True,           # FLANN with LSH
        ransac_reproj_threshold=5.0
    )

    print(f"\nConfiguration:")
    print(f"  Detector: BRISK")
    print(f"  AGAST threshold: {estimator.thresh}")
    print(f"  Octaves: {estimator.octaves}")
    print(f"  Pattern scale: {estimator.patternScale}")
